<!-- Context: standards/code | Priority: critical | Version: 2.1 | Updated: 2026-03-08 -->
# Code Standards
...
## Type Checking Policy (Python)

- **Blocking gate**: `mypy` must pass before merge.
- **Advisory signal**: `pyright` may be run for IDE parity and additional diagnostics but does not block merge.
- **Conflict handling**:
  - If mypy fails: must fix before proceeding.
  - If pyright warns and mypy passes: triage in-slice when low-cost; otherwise capture in lessons learned or technical debt.
- **Goal**: keep mypy as canonical CI gate while using pyright to align with strict Pylance workflows.
...
✅ Test in isolation
✅ Use mypy as blocking and pyright as advisory for Python typing
